...
| Threat hypothesis status | `Candidate / Supported / Partially Supported / Disconfirmed / Inconclusive` | `TH-2026-001`〜`005`で使用 |
...
`TH-2026-001`〜`003`は第1章の命題を保持する。第4章で分離した精密化は、継承IDを上書きせず、summary-only境界を`TH-2026-004`、App identity lifecycle Event / decision summary Fieldの観測を`TH-2026-005`として記録する。ここでdecision summary Fieldとは、`FLOW-2026-005`でAudit Evidenceを月末判断へ渡す無害化summaryのFieldであり、Admin consent EventやRuleそのものではない。`TH-2026-002`のStatus表記は、本Artifactの有限集合へ大文字小文字だけを正規化した`Partially Supported`とし、命題の意味は変更しない。
...
| `ASM-2026-002` | 合成manifest field inventoryはsummary-only境界の最小Data集合を十分代表している | Finance Data Owner | field inventoryとmanual import要件の比較 | 2026-08-15 | Assumed | `TH-2026-003`, `TH-2026-004`, `EREQ-2026-003` |
...
|---|---|---|---|---|---|---|---|---|
| `EREQ-2026-001` | 2026-07-25 remediation後のcurrent scope、Workload identity binding、Tenant binding、rotation手順は業務要件と一致するか | `TH-2026-001`, `TH-2026-004`, `CTRL-2026-005`, `CTRL-2026-006`, `GAP-2026-002` | post-remediation App registration export、要件表、scope差分表、Workload identity binding snapshot、Tenant binding差分、rotation手順Review記録 | 実Tokenを取得しない。実Dataを取得しない。Productionを変更しない。 | Platform | 2026-08-12 | Required | Historical inputs only（current resultではない）: `EVD-2026-001`, `EVD-2026-002`（いずれも2026-07-20）; New post-remediation result: current-scope snapshot、Workload identity binding snapshot、Tenant binding差分、rotation手順Review記録、offline機械的突合結果は未収集（承認後に新Evidence IDを割り当てる） |
| `EREQ-2026-002` | 同意EventとApp identity lifecycle Eventの監査Coverageは十分か | `TH-2026-002`, `TH-2026-005`, `CTRL-2026-007`, `GAP-2026-003` | 合成同意Event、合成App identity lifecycle Event、Audit export、両Event classのRule test結果 | 無害化summaryを超える追加Data exportを要求しない。新Authorization Record / RoE承認前にRule testを再実施しない。 | SOC | 2026-08-14 | Required | Coverage result: `EVD-2026-003`（Admin consent Eventのみ）; App identity lifecycle EventのCoverageと両Event classのRule test結果は未収集（承認後に新Evidence IDを割り当てる） |
| `EREQ-2026-003` | 保持範囲内で既往影響をどこまで評価でき、resource / operation Field補正後にControlを検証できるか | `TH-2026-001`, `TH-2026-003`, `TH-2026-004`, `CTRL-2026-009`, `GAP-2026-001`, `GAP-2026-003` | 90日窓のTelemetry summary、Coverage表、negative finding、retention note、resource / operation Field contract、承認後のpost-change telemetry result | PIIを収集しない。実Tenantへ接続しない。scope外Targetを追跡しない。Production変更や実Dataが必要な工程を第4章内で実行しない。 | SOC、Platform | 2026-08-18 | Required | Historical inputs only: `EVD-2026-004`, `NEG-2026-001`; resource / operation Fieldの実装記録とpost-change telemetry resultは未収集（承認済み運用工程後に新Evidence IDを割り当てる） |
...
|---|---|---|---|---|---|
| `HO-TM-2026-005` | 第5章 ATT&CK | Behavior記述 | `TH-2026-001`〜`005`の成立条件、Flow、Boundary、Exposure、観測点 | Technique名ではなく行動条件へ落とせる | Campaign名や主体帰属だけで具体性がない |
...
- 第5章では、継承命題`TH-2026-001`〜`003`、summary-only refinement `TH-2026-004`、lifecycle / summary Field refinement `TH-2026-005`を区別してATT&CKの行動言語へ変換する。
...
- [x] `TH-2026-001`〜`003`を第1章の意味を保って再利用し、summary-only refinementを`TH-2026-004`、lifecycle / summary Field refinementを`TH-2026-005`へ分離している
