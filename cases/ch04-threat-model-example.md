...
| `EREQ-2026-003` | 保持範囲内で既往影響をどこまで評価でき、resource / operation Field補正後にControlを検証でき、合成manifest field inventoryがmanual import要件を十分代表するか | `TH-2026-001`, `TH-2026-003`, `TH-2026-004`, `TH-2026-006`, `CTRL-2026-009`, `GAP-2026-001`, `GAP-2026-003` | 90日窓のTelemetry summary、Coverage表、negative finding、retention note、resource / operation Field contract、承認後のpost-change telemetry result、source fixture ID / version / hash付きmanifest / manual import field coverage matrix、全必須field coverage率、未解決の例外件数、Unknown件数、不一致件数、有限result（Passed / Failed / Inconclusive / Not collected）、Finance Data Owner sign-off | PIIを収集しない。実Tenantへ接続しない。scope外Targetを追跡しない。Production変更や実Dataが必要な工程を第4章内で実行しない。manifest代表性比較には合成fixtureだけを使用する。 | SOC、Platform、Finance Data Owner | 2026-08-18 | Required | Historical inputs only: `EVD-2026-004`, `NEG-2026-001`; resource / operation Fieldの実装記録とpost-change telemetry resultは未収集（承認済み運用工程後に新Evidence IDを割り当てる）。manifest representativeness comparison result（有限resultと各count）は未収集（`ACT-TM-2026-007`完了後に新Evidence IDを割り当てる） |
