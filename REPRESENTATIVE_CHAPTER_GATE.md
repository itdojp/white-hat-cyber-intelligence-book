...
| GATE-022 | P2 | 第1章のReview contractがdisclaimerとTechnical Evidence IDだけを検査し、TemplateのEvidence列や残り4観点の合成Evidence ID欠落を見逃した | 代表4 TemplateすべてのEvidence reference headerと、各合成Caseの5観点すべての`SYNTH-REV-*` IDを必須化した |
...
| GATE-038 | P2 | active raw HTML検出が行頭Tagだけを対象とし、Markdown文字列やblockquote markerの後にある`style` / `script` / `link`を見逃した | inline codeとescaped angleを除外した表示対象行全体からactive tagを検出し、行内位置にかかわらず禁止するようにした |
| GATE-039 | P2 | Source IDを含む本文block自体へKramdown IALを付けると、非表示Citationを本文使用として数えられた | 代表章・Artifact・Caseではblock IALを全面禁止し、Source citationを含む任意blockへvisibility-changing属性を付加できないようにした |
| GATE-040 | P2 | raw HTML container追跡が行頭Tagだけを対象とし、通常文字列の後にある`div hidden`で後続Review節を隠せた | inline codeとescapeをmaskした表示対象行全体をHTML parserへ渡し、行中位置にかかわらず未close containerを後続close tagまで追跡するようにした |
