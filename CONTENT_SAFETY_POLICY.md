...
有効なIPv4 literalに日本語case particleが直結する場合、IPv4の前に連続する日本語script、`へ / を / で`の三つ、およびその後ろのtoken終端または日本語scriptだけを有限なprose境界として扱う。reader-visible normalizationがinline markupを除去してIPv4を再結合した場合も、抽出したaddressをIP scannerへ明示的に引き渡す。この境界処理はIPv4の許可判断を変更せず、documentation rangeは引き続き許容し、non-documentation IPv4はIP scannerがrejectする。未知の助詞、ASCII混在prefix/tail、追加のdotted IDN/punycode label、または無効なIPv4はIDN候補としてfail closedにする。これはhost tokenizationの既存実装修正であり、category、Stable API、Policy meaningを変更しないためPolicy versionは`1.2.0`を維持する。
