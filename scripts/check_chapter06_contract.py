#!/usr/bin/env python3
"""Chapter6 Layer A selection/provenance/parity. Rendering belongs to Layer B."""
...
from scripts.chapter06_semantics import validate_case  # noqa: E402
...
DOCUMENTS = (
    ...
    "templates/signal-flow-diagram.md",
    "cases/ch06-signal-flow-example.md",
    "references/ch06-source-review-2026-09-12.md",
)
CASE_PATH = "cases/fixtures/ch06-signal-flow.json"
PARENT_PATH = "cases/fixtures/ch05-attack-behavior.json"
CONTRACT_PATH = "tests/fixtures/chapter06/publication-contract.json"
SOURCE_IDS = (
    "SRC-NIST-ZTA-001",
    "SRC-NIST-ZTAA-001",
    "SRC-NIST-LOG-001",
    "SRC-NIST-LOG-DRAFT-001",
    "SRC-IETF-OAUTH-CORE-001",
    "SRC-ATTACK-001",
    "SRC-ATTACK-DET-001",
)
...
def case_rows(data):
    ...
    for f in data["flows"]:
        title = f"{f['flowId']} — {f['coverage']}"
        expected += [
            f"{title} Field Value {k} {display(v)}"
            for k, v in f.items()
            if k not in ("nodes", "edges", "receipts", "test")
        ]
        expected += [f"Nodes ID Role {n['id']} {n['role']}" for n in f["nodes"]]
        expected += [
            f"Edges ID From To {e['id']} {e['from']} {e['to']}" for e in f["edges"]
        ]
        for r in f["receipts"]:
            expected += [
                f"{r['id']} Field Value {k} {display(v)}" for k, v in r.items()
            ]
        if f["test"]:
            expected += [
                f"Test Field Value {k} {display(v)}" for k, v in f["test"].items()
            ]
    return expected
...
def document_errors(document, spec, data):
    ...
    if document.document_id == DOCUMENTS[2]:
        ...
        if actual != case_rows(data):
            errors.append("ART16 JSON/projected case parity")
    if document.document_id == DOCUMENTS[0]:
        ...
        if body != set(SOURCE_IDS) or refs != set(SOURCE_IDS):
            errors.append("Chapter6 visible Source body/reference inventory")
    return errors
...
def json_safety(value, location=CASE_PATH):
    ...
    if isinstance(value, str):
        # Reviewed neutral metadata description, not an instruction to collect PII.
        # It is confined to this field in the closed six-flow teaching set.
        if (
            location == CASE_PATH + "/flows/2/operationPurpose"
            and value == "Customer Data APIの合成利用記録"
        ):
            return []
        ...
        if any(c in value for c in "<>&`\\"):
            return [f"{location}: ART16 plain-text data only"]
        return [
...
def repository_errors(contract):
    ...
    if (
        package.get("check:chapter06") != "python3 scripts/check_chapter06_contract.py"
        or package["test"].split(" && ").count("npm run check:chapter06") != 1
    ):
        errors.append("root Chapter6 invocation")
    if not package["sync:docs"].startswith(
        "python3 scripts/check_chapter06_contract.py --no-regressions && "
    ):
        errors.append("Chapter6 publication preflight")
    registry = load_json_strict(ROOT / "site-pages.json")
    for k in ("pages", "staticFiles"):
        for item in contract[k]:
            if registry[k].count(item) != 1:
                errors.append("Chapter6 publication route " + item["source"])
    sources = load_json_strict(ROOT / "references/sources.json")["sources"]
    if {s["id"] for s in sources if 6 in s["chapters"]} != set(SOURCE_IDS):
        errors.append("Chapter6 Source mapping")
    for sid in SOURCE_IDS:
        ...
        if not meets_audit_baseline(
            ...
            errors.append("Chapter6 scoped Source audit " + sid)
...
def main():
    ...
    try:
        ...
        if not args.no_regressions:
            from scripts.chapter06_regressions import run_regressions

            count, tests = run_regressions(data, parent, contract, source)
            errors += tests
        if errors:
            for e in errors:
                print("ERROR:", e)
            return 1
        print(
            f"Chapter 6 contract passed: 4 complete documents; 6 ART-16 flows; {count} regressions; Policy {POLICY_VERSION}; Projection {PROJECTION_VERSION}"
        )
        return 0
    except (
        OSError,
        ...
        print("ERROR: Chapter6 contract:", exc)
        return 1
