...
def main() -> None:
    ...
    revision = json.loads((ROOT / ".book-formatter/revision.json").read_text())
...
assets/apple-touch-icon.png
    ...
    with tempfile.TemporaryDirectory(prefix="mermaid-contract-", dir=tmp) as directory:
        ...
        (node / "package.json").write_text(json.dumps({"name": "@mermaid-js/tiny", "version": MERMAID_VERSION}))
        ...
        (node / "package.json").write_text('{"name":"@mermaid-js/tiny","version":"0.0.0"}')
        try:
            load_publication_assets(root)
        except ValueError:
            pass
    ...
    fixtures = json.loads((ROOT / "tests/fixtures/mermaid/publication.json").read_text())
    ...
    assert (ROOT / "node_modules/@mermaid-js/tiny/LICENSE").read_text().strip() in notice
