...
- Chapter 2本文のH1 / document preambleから参考文献・Source Note IDまで、`ART-13 Authorization Checklist`と合成CaseのH1 / document preambleから全Record sectionまでを、有限なreader-visible fieldとして選択する。Markdown paragraph / list item内のsoft wrapとfield内の`<br>`は表示上のspaceへ投影し、各本文fieldは所属するATX heading階層とも関連付けて検査する。
