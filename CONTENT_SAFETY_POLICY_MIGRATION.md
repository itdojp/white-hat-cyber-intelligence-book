...
- Kramdown / Jekyllのrender後にsource fieldとの意味差が生じるraw HTML、Liquid、definition list、IAL、footnoteはfail-closedで拒否する。raw HTMLは`<br>`だけを許可し、それ以外は同等のMarkdown heading / list / tableを使用する。Markdown link / autolink / reference linkの実行可能URL schemeも拒否する。
