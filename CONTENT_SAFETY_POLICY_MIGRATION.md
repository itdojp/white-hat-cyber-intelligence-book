...
- Kramdown / Jekyllのrender後にsource fieldとの意味差が生じるraw HTML、HTML comment、Liquid、definition list、blockquote、IAL、footnote、abbreviation、math、single-line / multiline underscore emphasis、reference-style link、escaped link opener、empty-label / empty-destination / balanced-parenthesis-destination / escaped-parenthesis-destination link / image、destination内backtick、reader-visibleなlink / image titleはfail-closedで拒否する。JekyllがMarkdownより先に評価するLiquidはunmasked sourceで拒否する。raw HTML tag内のbacktickはinline-code opening delimiterから除外し、active tagをmaskしない。fenced / indented / inline codeはその他のrender-time guardからmaskする一方、comment / angle delimiterをPolicy用に投影したreader-visible payloadとしてscanを維持する。非tag / 非autolinkのbare-angle bodyに加え、HTML character reference decode / NFKC正規化によって初めて生じるangle delimiterのbodyもreader-visible fieldとして追加する。raw HTMLは属性なしの`<br>` / `<br/>` / `<br />`だけを許可し、それ以外は同等のMarkdown heading / list / tableを使用する。Markdown link / autolinkの実行可能URL schemeと、decode / 正規化後のHTTP(S) / scheme-relative special URL内に残るbackslashも拒否する。
