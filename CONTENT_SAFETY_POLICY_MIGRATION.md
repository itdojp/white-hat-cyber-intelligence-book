...
- Chapter 2本文のH1 / document preambleから参考文献・Source Note IDまで、`ART-13 Authorization Checklist`と合成CaseのH1 / document preambleから全Record sectionまでを、有限なreader-visible fieldとして選択する。Markdown paragraph / list item内のsoft wrap、field内の`<br>`、character reference由来の空白は表示上のspaceへ投影し、two-space / backslashによるsource hard breakは隣接するreader-visible tokenを分断しない形へ投影する。pipe-prefixed lineはdelimiter rowでtableと確定した場合だけ独立fieldとする。0-3 space indentを許容したATX / Setext headingをsection inventoryと関連付けへ含める。各本文fieldは所属するheading階層、nested list itemとcontinuation paragraphは祖先item、table body rowはheader rowとも関連付けて検査する。
- backtick / tildeによるfenced codeと4-space / tabによるindented codeは、空行を含むreader-visible payload全体を単一fieldとして検査する。backtick-fence info string内のbacktickは拒否する。inline code opening delimiterはbackslash escape parityを、closing delimiterは同一run長をfull source上で確認し、multiline payloadを一つのfieldとして検査する。list内のindented code判定とterminal continuationは最内の適用可能なancestor itemを解決し、そのcontent indentationへ4 source columnを加えた閾値を用いる。
- Kramdown / Jekyllのrender後にsource fieldとの意味差が生じるraw HTML、HTML comment、Liquid、definition list、blockquote、IAL、footnote、abbreviation、math、underscore emphasis、reference-style link、empty-label link / image、reader-visibleなlink / image titleはfail-closedで拒否する。JekyllがMarkdownより先に評価するLiquidはunmasked sourceで拒否する。fenced / indented / inline codeはその他のrender-time guardからmaskする一方、comment / angle delimiterをPolicy用に投影したreader-visible payloadとしてscanを維持する。非tag / 非autolinkのbare-angle bodyもHTML character reference decode / NFKC正規化後のreader-visible fieldとして追加する。raw HTMLは属性なしの`<br>` / `<br/>` / `<br />`だけを許可し、それ以外は同等のMarkdown heading / list / tableを使用する。Markdown link / autolinkの実行可能URL schemeと、decode / 正規化後のHTTP(S) / scheme-relative special URL内に残るbackslashも拒否する。
